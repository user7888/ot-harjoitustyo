...
class TestMap(unittest.TestCase):
    def setUp(self):
        ...
        self.map = Map(LEVEL_MAP, CELL_SIZE, StubDisplay(), Controller(), Player())
    ...
    def test_monster_can_be_moved(self):
        ...
        self.assert_coordinates_equal(
            monsters[0], 0 * CELL_SIZE, 0 * CELL_SIZE)
        # Move monster 1 CELL_SIZE downwards.
        self.map.move_monster_cell(dy=+CELL_SIZE)
        self.assert_coordinates_equal(
            monsters[0], 0 * CELL_SIZE, 1 * CELL_SIZE)
        # Move monster 1 CELL_SIZE to right.
        self.map.move_monster_cell(dx=+CELL_SIZE)
        self.assert_coordinates_equal(
            monsters[0], 1 * CELL_SIZE, 1 * CELL_SIZE)
